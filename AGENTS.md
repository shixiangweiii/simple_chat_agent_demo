...
- `_resume_inner(session_id, state, decision, answer, is_disconnected)` — async generator,按 awaiting tool 的 name + decision 构造 tool result(`ask_user → answer` 文本;`execute_shell_command + approve → "[demo stub] 已模拟执行命令: ..."` 字符串,**demo 不真执行 shell**,教学焦点是 HITL 流程不引入真实 RCE 风险;`reject → "用户拒绝执行。理由: ..."`),append role=tool 到 messages,yield 一条 `tool_result`,然后委派给 `_stream_react_rounds`(传入 `remaining_tool_calls` 作为 `pending_remaining`)继续。
