...
| 长期记忆 (Phase 11) | `longterm_memory.py` | 跨会话长期记忆 store + 抽取/协调/注入/检索:`ingest_async`(归档时摄入) / `retrieve_injection_async`(query-aware 注入) / `injection_fragment`(同步全量) / `snapshot` / `delete_fact_async`;依赖 `llm_client` 的 `complete[_async]` / `embed_texts[_async]` / `rerank[_async]` | Importing chat_core / fastapi / mcp_web_search, knowing about Memory 类 / SSE / ReAct / Sessions |
...
2. `POST /api/chat` returns `text/event-stream`. The HTTP layer calls `chat_core.stream_agent_response()` (which mirrors `react()` but yields abstract `(event_name, payload_dict)` tuples) and wraps the stream in `_sse_stream` to format each tuple as an SSE frame via `sse(event, data)` (`_sse_stream` also injects AG-UI/A2UI protocol tag fields via `chat_core._with_protocol_tags`, Phase 7b). The handler uses `chat_core.get_or_load(sid)` so even a brand-new server can lazy-load a Memory from disk archive when an existing session_id sends in a chat request. **Every SSE-streaming route** (`/api/chat`, `/api/resume`, `/api/ui_action`, `/api/plan_confirm`, `/api/plan_decision`, `/api/plan_continue`) wraps its stream in a per-session `asyncio.Lock` from `chat_core.get_session_lock(sid)` (Phase 7a M6) so two streams for one session can never interleave shared-state mutations; `/api/steer` deliberately does **not** take this lock (it must run while a stream is active). Each of these routes also calls `_reject_if_busy(sid)` **before** invoking the business sync-shell (which mutates `_PENDING` / plan state outside the lock): if the session lock is already held by an active stream, the route returns **409** immediately rather than mutating state another stream depends on. Residual race: a request arriving after the active stream's route returned but before its `_locked()` generator acquired the lock will pass the `lock.locked()` check; both then serialize on the lock — strictly tighter than mutating bare outside the lock, but not a full guarantee. Acceptable for the demo; a stricter design would acquire the lock before any state mutation.
...
- `_build_native_tools[_async]()` — 模块级懒加载,首次调用走 `mcp_web_search.discover_tool_spec[_async]()`(列出 MCP server 暴露的所有工具的 OpenAI tools 格式 spec),并加入 `RENDER_UI_TOOL` / `UPDATE_UI_DATA_TOOL`(本地立即工具,即 `IMMEDIATE_LOCAL_TOOLS`) + `LOCAL_TOOLS.values()`(HITL 伪工具),合并结果缓存到 `_NATIVE_TOOLS_CACHE`。
...
**Parse-failure semantics**: `Memory.from_markdown` raises on parse failure (empty text / no turn markers just yields an empty Memory). Callers deliberately do **not** cache an empty Memory on failure — `get_or_load` returns a temporary uncached empty Memory and `read_history` raises `HistoryNotFound` — so the next request retries the disk read instead of permanently shadowing the user's archive with an empty session.

**Persistence touch points**:

| When | What happens |
|---|---|
| User clicks "归档" | `POST /api/archive` calls `chat_core.archive_session()` which covers-writes the file (atomic via `.tmp` + `os.replace`). Empty Memory is silently skipped. Phase 7a M5: archiving also prunes completed plans and processed answer drafts from runtime state (active pending and UI surfaces are kept). |
...
| User clicks "重置" | `POST /api/reset` calls `chat_core.reset_session()` which drops the session from `sessions` AND deletes the archive file. session_id is preserved. Also evicts the session's `_STEER_QUEUES` / `_STEER_HISTORY` / `_SESSION_LOCKS` entries and the runtime sidecar. |
| User deletes session in sidebar | `DELETE /api/sessions/{id}` calls `chat_core.delete_session()` which removes the file + drops in-memory session (same in-process-state eviction as reset). |
...
| Runtime state mutation | `chat_core` saves `_PENDING` / `_UI_SURFACES` / `_PLANS` to JSON via atomic write; corrupt or incompatible sidecars are logged and ignored. On restore (服务重启路径), a plan stuck in `running` is downgraded to `paused` **and its first unfinished step is marked `error`** ("服务重启，步骤执行被中断"), so the frontend shows the **plan-decision card** and the user resumes via `POST /api/plan_decision` (`retry`/`skip`/`update`) — **not** `/api/plan_continue` (that route requires `status=="running"` and only serves the in-process "stream dropped but plan still running" case, e.g. browser closed mid-stream without a restart). `_RUNTIME_RESTORED.add` happens at function end so a partial failure retries on the next request (Phase 7a M7). |
...
在"每个 session 隔离的对话 Memory"之上叠加一层 **mem0 式的全局长期记忆**:从每个会话归档时批量抽取"关于用户的原子事实",经 LLM 协调并入全局记忆库,在后续任意会话注入到 system prompt,让 Agent 跨会话"记得用户"。**不改 session 隔离的基本面**(Memory 仍是 flat-string,归档格式不变);记忆库是与 session 解耦的**独立 store**。

新模块 `demo/longterm_memory.py` 在 `chat_core` 下层(`chat_core → longterm_memory → llm_client`),**不** import `chat_core`(无环),只吃原语(`turns: list[{role,msg}]` / `query: str`)。embedding/rerank/无工具补全作为"模型调用"归 `llm_client`(`complete[_async]` / `embed_texts[_async]` / `rerank[_async]`);web 层只依赖 `chat_core` 的 3 个薄 re-export(`schedule_longterm_ingest` / `longterm_snapshot` / `delete_longterm_fact`)。

**存储**(git-ignored,与 chat_archive / runtime_state 同级):`data/longterm_memory.json`(人类可读的事实库 `{version, facts:[{id,text,category,created_at,updated_at,source_sessions}], ingested:{sid:turn_count}}`)+ `data/longterm_vectors.json`(模拟向量库 `{embed_model, dim, vectors:{fact_id:[floats]}}`)。全局 `_LTM_LOCK` 串行所有读-改-写(facts + vectors 同锁);读路径(snapshot / retrieve)不取锁,靠原子写 + 检索侧降级吸收跨文件瞬时不一致。

**数据流**:`/api/archive` 成功 → `schedule_longterm_ingest(sid)`(快照 turns 后 `create_task`,立即返回)→ `ingest_async`(`_LTM_LOCK` + 增量水位线去重 + 异常隔离不推进水位线):`extract_facts_async`(无工具补全抽候选)→ `reconcile_async`(已有事实 + 候选 → ADD/UPDATE/DELETE/NOOP,**校验幻觉 id**)→ `apply_ops` → `_sync_vectors`(ADD/UPDATE 嵌入、DELETE 删向量,best-effort)→ 原子写 + 推进水位线。下次任意 fresh turn(`/api/chat` 与 `/api/ui_action` 经 `stream_agent_response`;CLI 经 `react`):`retrieve_injection_async(query)` —— 事实 ≤ `LTM_INJECT_ALL_MAX`(默认 12)全量注入,否则 cosine 召回 top-N → qwen3-rerank 精排 top-K;结果**合并进现有 `adaptive_fragment` 通道**进 system prompt(零下游签名改动)。**resume / plan 流不重注入**(LTM 片段已烘焙进 fresh turn 的 system message,随 `_PENDING["messages"]` / `execution_messages` 复用)。

**优雅降级阶梯(检索绝不阻断聊天)**:rerank 失败 → cosine 序前 K;query embedding 失败 → 注入最近更新 K 条;向量缺失/头(model/dim)不匹配 → 全量注入截断;任何异常 → 最近事实兜底。`retrieve_injection_async` 全程不抛。

**API 无关性**:抽取走无工具 chat 补全、注入走 system prompt 片段 → `responses` / `chat` 两模式都生效(与 Phase 3–10 全 chat-only 对照)。

**配置**(env 覆盖):`LTM_MIN_TURNS=2`、`LTM_MAX_FACTS=200`、`LTM_MAX_INJECT_CHARS=2000`、`LTM_INJECT_ALL_MAX=12`、`LTM_RECALL_TOP_N=20`、`LTM_RERANK_TOP_K=6`、`QWEN_EMBED_MODEL=text-embedding-v4`、`QWEN_EMBED_DIM=512`(可调小省体积)、`QWEN_RERANK_MODEL=qwen3-rerank`(gte-rerank 2026-05-30 下线)。embedding 走 OpenAI 兼容 `compatible-mode/v1`(复用 LLM client 单例);rerank 是裸 httpx POST 到 `compatible-api/v1/reranks`(不同路径,非 OpenAI SDK 方法,失败返回 None 让上层降级)。`complete[_async]` 用 `QWEN_LTM_MODEL`(默认 = 主 `MODEL`,成本敏感可单独配更便宜模型)+ `QWEN_LTM_COMPLETE_TIMEOUT`(默认 60s)/ `QWEN_LTM_EMBED_TIMEOUT`(默认 30s)硬超时,把 ingest 持 `_LTM_LOCK` 的最坏时长从 client 默认(分钟级)压到秒级。

**性能/一致性**:读路径(每轮 chat 的注入、snapshot)走进程内缓存 `_CACHE_FACTS` / `_CACHE_VECTORS`,避免反复同步解析整文件(向量文件可 MB 级)。一致性靠 copy-on-write —— **写路径用 `_load_disk` / `_load_vectors_disk` 读盘新对象去 mutate,`_save` / `_save_vectors` 写盘后整体替换缓存引用**,读者拿到的永远是某次 save 定格的不可变对象,写者从不原地改它,故 lock-free 读安全。单进程是唯一写者,不做 mtime 失效(外部手改文件需重启进程)。

**摄入健壮性**:抽取/协调的 JSON 解析区分"真·空"(没数组 / `[]` → 推进水位线)与"坏 JSON"(有 `[` 但解析失败 → 抛 `_LTMParseError` → ingest 不推进水位线、下次归档重试),避免把瞬时坏输出误判成"没事实"而永久丢 delta。后台摄入 task 在 `chat_core._LTM_INGEST_TASKS` 持强引用直到完成(防 asyncio 弱引用 GC 中途取消)。

**安全/隐私(重要边界)**:长期记忆是**单一全局用户**假设 —— `GET /api/longterm_memory` 全局无鉴权返回全部事实,且从任一会话抽取的"事实"会注入**所有**后续会话的 system prompt,构成一条**跨会话持久提示注入面**(恶意/误导内容沉淀后影响未来所有对话)。单用户本地 demo 下用户只能"影响自己",可接受;**多用户 / 共享部署必须**:按 user 隔离记忆库、对抽取出的事实做审核/过滤、给只读端点加鉴权。本 demo 不做这些(non-goal)。
...
| `GET` | `/api/health` | `{ok, model, sessions, api_mode}` — 前端用 `api_mode` 决定是否启用 steer 按钮 |
...
| `POST` | `/api/chat` | SSE stream (see contract below). Body: `{session_id, message, context?, images?, attachments?}`, context 含 `viewport_width` / `selected_text` / `session_message_count`;**Phase 10a** `images: list[str] | null` 为可选 base64 data URL 列表(≤3 张 / 每张 ≤5MB / 必须 `data:image/*`),仅 `API_MODE=chat` 支持;校验失败 400 `ImagePayloadInvalid`。**Phase 10b** `attachments: list[dict] | null` 为可选文本附件列表(≤5 个 / 单文件硬上限 100KB / 总量硬上限 100KB / mime 严格白名单),仅 `API_MODE=chat` 支持;校验失败 400 `AttachmentPayloadInvalid`。 |
...
| `POST` | `/api/ui_action` | Declarative UI button action:body `{session_id, surface_id, component_id, event_name, form_data?}`(**Phase 8b** `form_data` 是前端 surface.data 快照,含表单字段值,后端 merge 进 surface data 并注入给模型),校验内存态 surface/action 后启**新**SSE 流继续 ReAct。仅 `API_MODE=chat` 可用;404 = surface/action 不存在;409 = event_name mismatch。 |
...
| `POST` | `/api/plan_continue` | 恢复**仍处于 `running`** 的 plan(典型:进程未重启、SSE 流断开但 plan 内存态还在 —— 如浏览器中途关闭):body `{session_id, plan_id}`,启**新**SSE 流继续逐步执行。要求 `status=="running"`,否则 409。**服务重启场景不走这里**:重启后 sidecar 恢复会把 `running` 降级成 `paused` + 中断步标 `error`,改由 `/api/plan_decision`(retry/skip/update)恢复。错误码同 plan_confirm。 |
...
| `POST` | `/api/steer` | **Phase 9a** Agent steering:body `{session_id, message}`,把 message 入队到 `_STEER_QUEUES[session_id]`,**立即返回** dict(非 SSE 流);由**当前活跃流或下一条 `/api/resume`、`/api/plan_*` 流**在轮次边界(`_stream_react_rounds` / `_stream_plan_step_rounds` 顶部 drain)消费。**注意**:新发起的 `/api/chat` 会主动丢弃残留未消费的 steer(`_stream_chat_native` Phase 9 fix B2),避免过期纠偏污染新对话 —— 故返回的 `{queued:true}` 不保证一定被消费。**不获取** session lock(否则与活跃 SSE 流死锁)。仅 `API_MODE=chat` 可用;400 = 非 chat 模式或 message 为空。 |
| `POST` | `/api/reset` | Clear current session memory + delete archive; session_id preserved |
| `POST` | `/api/archive` | Cover-write current Memory to disk; empty → `{ok, skipped:true}`. **Phase 11**: 归档成功(未 skip)后**后台**触发 `chat_core.schedule_longterm_ingest(sid)`(create_task,不阻塞本响应)。 |
| `GET` | `/api/longterm_memory` | **Phase 11** 跨会话长期记忆只读快照 `{count, facts:[{id,text,category,created_at,updated_at,source_sessions}]}`,供前端「🧠 记忆」面板渲染。 |
| `DELETE` | `/api/longterm_memory/{fact_id}` | **Phase 11** 删除一条长期记忆事实。fact_id 形态非法 → 400。 |
...
| `begin_rendering` | `{surface_id, root, catalog_id}` | Phase 7b/8a 流式渲染确认信号(A2UI beginRendering 语义)。仅当 `render_ui` 以 `complete=false` 增量模式调用时发,前端据此移除占位符并标记 surface 已确认。 |
| `steer_applied` | `{message, round}` | Phase 9a steering 消息被 drain 进 messages 时发,前端把对应排队中的 steer 标记为已生效。 |
| `agent_state_snapshot` | Agent 全局态纲要(round / tool_stats / surfaces / plans / pending / steer_history) | Phase 9b。**仅 `/api/chat` 与 `/api/ui_action`**(后者复用 `stream_agent_response`)流开头各发一次(仅 `chat` 模式);`/api/resume` 与各 plan 流**不发**,前端 reducer 在这两条流上靠 `agent_state_delta` 增量维护。与 `activity_snapshot`(plan 作用域)互不替代。 |
| `agent_state_delta` | `{patch}` — RFC 6902 JSON Patch,仅用 `replace` 与 `add(/array/-)` 两种 op | Phase 9b。round 推进 / tool_stats / steer_history 变化时发;前端 reducer `agentStateStore.applyPatch` 也仅支持这两种 op。 |
...
- **Phase 11 长期记忆是与 session Memory 正交的独立 store** —— `longterm_memory.py` 不读写 Memory / 归档 md / runtime_state,只读写 `data/longterm_memory.json` + `data/longterm_vectors.json`。**不要**把长期记忆并进 Memory(那条 flat-string non-goal 不变),也**不要**让 `longterm_memory` import `chat_core`(会成环 —— 它只吃 `list[{role,msg}]` / `str` 原语)。新增事实 category 在 `longterm_memory._CATEGORIES` + 前端 `LTM_CAT_LABELS` 两处同步;调检索行为改 `LTM_*` env 或 `_retrieve` / `retrieve_injection_async`(注意保持降级阶梯,任何分支都返回可用片段、绝不抛);抽取/协调质量改 `_EXTRACT_PROMPT` / `_RECONCILE_PROMPT`;embedding/rerank 模型改 `QWEN_EMBED_MODEL` / `QWEN_RERANK_MODEL` env(rerank 走裸 httpx,换模型注意 endpoint 路径与响应结构可能不同 —— 见 `docs/外部接口文档/排序模型（Rerank）.md`)。注入复用 `adaptive_fragment` 通道,新增 system-prompt 附加片段时优先走同一通道而非加签名。
- **Layer discipline**: imports flow downward only (entry → `chat_core` → `llm_client` / `mcp_web_search` / `longterm_memory`). Never let `web_chat_agent` or `common_chat_agent` import `llm_client` or `mcp_web_search` directly (use the `MODEL` / `API_MODE` re-exports on `chat_core`); never let `chat_core` import `fastapi` / `starlette` or raise `HTTPException` (raise a domain exception — `InvalidSessionId`、`HistoryNotFound` / `PendingNotFound` / `PendingMismatch`、`Ui*` / `Plan*` / `DraftNotFound` / `SteerUnavailable` 同族 — and let the HTTP layer translate; convention: `ValueError` 子类 → 400/409,`LookupError` 子类 → 404); never let `llm_client` or `mcp_web_search` import each other or `chat_core`. Business changes (Memory / ReAct / Sessions / Tools / HITL pending) live in `chat_core`; model invocation / chunk parsing / new SSE-event-bearing data extracted from chunks live in `llm_client`; MCP wire protocol / schema 转换 lives in `mcp_web_search`; HTTP routing / SSE serialization (`sse()` + `_sse_stream`) / Pydantic models / domain-exception translation live in `web_chat_agent`.
...
- **Phase 3 声明式 UI (`render_ui` / `update_ui_data`,仅 `chat` 模式生效)**:`RENDER_UI_TOOL` 与 `UPDATE_UI_DATA_TOOL` 不放入 HITL `LOCAL_TOOLS`,而是通过 `IMMEDIATE_LOCAL_TOOLS` 本地立即执行。`render_ui` 参数为 `{surface_id, components, data?}`,其中 `components` 是扁平数组且必须包含 `id="root"`;`button.action` 约定为 `{event_name, context?}`。后端把 surface/action 存进 `_UI_SURFACES`,不进入 Memory / markdown 归档;Phase 5 会把它同步到 runtime sidecar 以支持刷新/重启恢复。前端 `DeclarativeRenderer` 支持 `text/card/row/column/table/button` + Phase 8b 表单组件 `text_field/select/toggle`;所有文本用 `textContent`,不执行 HTML/JS。带 action 的 button 点击 `POST /api/ui_action`,新启 SSE 流继续 ReAct;后端只信任 registry 中保存的 action context。`update_ui_data(surface_id, path, value)` 按 JSON Pointer 更新 data 并发送 `ui_data_update`。CLI 遇到本地立即 UI 工具只喂回 Web-only 文本结果,不渲染 UI。
- **Phase 8 声明式 UI 增量与表单(仅 `chat` 模式生效)**:
  - **8a 流式增量**:`render_ui` 增加 `merge` / `complete` 参数。`complete=false` 走按 `component.id` 的增量合并(后续调用可不带 `root`,只要已存在的 surface 含 root),并在结果里附带 `begin_rendering` 信号(A2UI beginRendering 语义)让前端确认首次渲染;`complete=true` 保持覆盖语义。
  - **8b 表单组件**:`text_field`(`input_type: shortText|longText|number`)/ `select`(多选用 checkbox 组)/ `toggle`,均通过 `value_path`(JSON Pointer)双向绑定 surface.data。后端 `_validate_form_components` 做 schema 校验,失败立即返回错误,前端不渲染半成品;`update_ui_data` 支持 `auto_create=True` 自动创建中间路径。表单提交即带 action 的 button:前端把 surface.data 整体作为 `form_data` 随 `/api/ui_action` 回传,后端 merge 进 surface data 并把 `form_data` JSON 注入给模型。CLI 遇到含表单组件的 `render_ui` 短路(`_render_ui_has_form_components`)。
