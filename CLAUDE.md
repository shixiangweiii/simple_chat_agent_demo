...
| Runtime state mutation | `chat_core` saves `_PENDING` / `_UI_SURFACES` / `_PLANS` to JSON via atomic write; corrupt or incompatible sidecars are logged and ignored. On restore, a plan stuck in `running` is downgraded to `paused` so the frontend can offer "继续执行" → `POST /api/plan_continue` (Phase 7a M4); `_RUNTIME_RESTORED.add` happens at function end so a partial failure retries on the next request (Phase 7a M7). |
...
| `POST` | `/api/plan_continue` | 恢复 `paused` plan(典型:服务重启后 runtime sidecar 把 `running` 恢复成 `paused`,前端显示"继续执行"):body `{session_id, plan_id}`,启**新**SSE 流继续逐步执行。错误码同 plan_confirm。 |
...
| `agent_state_snapshot` | Agent 全局态纲要(round / tool_stats / surfaces / plans / pending / steer_history) | Phase 9b。流开头发一次(仅 `chat` 模式)。与 `activity_snapshot`(plan 作用域)互不替代。 |
...
- **Phase 7 健壮性 & 协议标签**:
  - **7a 并发与恢复加固**:每 session 一把 `asyncio.Lock`(`get_session_lock`),所有 SSE 流路由在 HTTP 层 `async with lock` 串行化(`/api/steer` 除外,见 Phase 9);`reset_session` / `delete_session` 同步清理 `_SESSION_LOCKS` / `_STEER_QUEUES` / `_STEER_HISTORY`;runtime sidecar 恢复时 `running` plan 降级为 `paused`(可经 `/api/plan_continue` 继续),恢复失败可重试;归档时清理已完成 plan / 已处理 draft。
