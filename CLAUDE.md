...
| `POST` | `/api/chat` | SSE stream (see contract below). Body: `{session_id, message, context?, images?}`, context 含 `viewport_width` / `selected_text` / `session_message_count`;**Phase 10a** `images: list[str] | null` 为可选 base64 data URL 列表(≤3 张 / 每张 ≤5MB / 必须 `data:image/*`),仅 `API_MODE=chat` 支持;校验失败 400 `ImagePayloadInvalid`。 |
